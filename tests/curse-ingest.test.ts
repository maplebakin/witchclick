...
describe('ingestCurse CLI helper', () => {
  ...
  it('writes curse markdown to archive directory', async () => {
    const spec = {
      specVersion: 1,
    ...
    const files = fs.readdirSync(path.join(tmpDir, 'archive', 'curses'));
    expect(files.length).toBe(1);
    const firstFile = files[0];
    if (!firstFile) {
      throw new Error('Expected generated curse markdown file');
    }
    const saved = fs.readFileSync(path.join(tmpDir, 'archive', 'curses', firstFile), 'utf8');
